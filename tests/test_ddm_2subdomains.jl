...
filter1(x,y) = 0*x + (y.<=2*h) + (y.>2*h).*(y.<y1[end-2]).*exp(-k^(3/2)/100*(y-2*h).^2)
filter2(x,y) = 0*x + (y.>=-h) + (y.<-h).*(y.>y2[3]).*exp(-k^(3/2)/100*(y+h).^2)
...
Mapproxsp1 =  (As + k^2*(AG*spdiagm(nu1(X,Y))))[indVol1,indVol1];
...
Mapproxsp2 =  (As + k^2*(AG*spdiagm(nu2(X,Y))))[indVol2,indVol2];
...
function MinvAp(x)
    ...
    info =  gmres!(y, ApplyPrecond, precondIT(x), tol = 1e-8)
    println("number of iterations for inner solver is ", countnz(info[2].residuals[:]))
    return y
end
