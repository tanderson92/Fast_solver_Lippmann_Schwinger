...
#@inline function Gtruncated2D{T,N}(L::Float64,k::Float64,s::Array{T,N}) where {T,N}
@inline function Gtruncated2D(L::Float64,k::Float64,s::Array{T,N}) where {T,N}
    return @. (1 + (1im*pi/2*L*hankelh1(0,L*k))*(s.*besselj.(1,L*s)) - (1im*pi/2*L*k*hankelh1(1,L*k))*besselj(0,L*s)  )./(s.^2 - k^2)
end


@inline function Gtruncated3D(L::Float64,k::Float64,s::Array{T,N}) where {T,N}
