...
for ll = 1:length(H)
    ...
    for kk = 1:length(tolerance)
        innerTol = tolerance[kk]
        Precond = PolarizedTracesPreconditioner(As, SubArray, nIt = 1 ,tol = innerTol);
        ...
        for ii = 1:length(theta)
            ...
            tic();
            info  = gmres!(u, fastconv, rhs, Precond, tol = 1.e-10)
            time += toc();
        ...
        println("Solving the negative smooth wavespeed using Integral formulation")
        println("Frequency is ", k/(2*pi))
        println("Number of discretization points is ", 1/h)
        println("Number of Subdomains is ", nSubdomains)
